...
Overall, cross fostering induces an upregulation of lipid metabolic, transport, and bindings, functions in epithelial cells, as well as PPARg singling, all pointing towards increased fatty acid metabolism and, absorption. This signature is evident when looking at all epithelial cells together. Or when looking at stem cells and transit amplifying cells in particular.Furthermore, That is a reduction. On a specific goblet cell cluster. That is characterized by more mature or secretory gene programs, while a different goblet cell cluster (with perhaps more antimicrobial or induced functions) is not affected.
...
plot <- grid.arrange(grobs = p)
# ggsave('initial_clustering_results_by_resolution.pdf', path = here('result/figuresto
# '), plot = plot)
```
...
source(here('scripts/function_template.r'))
seurat <- PrepSCTFindMarkers(seurat)
plot1 <- top_genes_per_cluster(seurat, object_annotations = object_annotations, tables_path = tables_path, figures_path = figures_path, results_path = results_path, run_pathway_enrichment = T) 
```
...
Overall, there are very few changes in stromal cells upon cross fostering. Perhaps there is a small signature of lipid metabolism or transport, although that could be contaminating epithelial ambient RNA or leftover epithelial cells/doublets, as the UMI counts are low. Furthermore, the upregulation of fibroblast-related genes in enteric glia could be a cluster contamination as well. 
